  ...
  return (
    <div className="dashboard-card">
      ...
      <div className="flex justify-around items-center py-4 sm:py-6 px-1 sm:px-4 overflow-x-auto">
        <GaugeChart value={stats.freshRequired} maxValue={maxValue} label="Fresh Required" color="gray" onClick={() => setActivePopup('freshRequired')} percentage={stats.total > 0 ? (stats.freshRequired / stats.total) * 100 : 0} />
        <GaugeChart value={stats.active} maxValue={maxValue} label="Active" color="cyan" onClick={() => setActivePopup('active')} percentage={stats.total > 0 ? (stats.active / stats.total) * 100 : 0} />
